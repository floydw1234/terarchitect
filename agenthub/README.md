...
Terarchitect's Phase 1 shipping vocabulary is intentionally DAG-native on top of this model: `shipped_frontier` names the already-shipped DAG commit, accepted `TicketAttempt`s name per-ticket accepted work, promotion candidates name stable operator-reviewed selections of accepted attempts, and `ShipRun`s remain execution-only shipping records. AgentHub itself stays neutral and does not encode Terarchitect promotion semantics.
...
| GET | `/api/events` | Recent typed events (`?channel_prefix=ticket-|ship-|cand-`) |
