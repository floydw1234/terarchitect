...
5. **Fri 2026-09-11 — dogfood-loop blocker fix.** Close one concrete blocker preventing a full dogfood loop on a real local project (reviewFeed or similar): choose-winner → accept-winner → compose promotion candidate → ShipRun → release PR → `shipped_frontier` advances. Identify the specific API/behavior gap and fix it. Verify: end-to-end test or CLI/`ta` run on the spark checkout (`/home/william/Documents/codingProj/terarchitect`) — no browser or UI required. Status: `todo`.
