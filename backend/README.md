...
When a ticket is moved to "In Progress", the app inserts a row into `agent_jobs`. A **coordinator** claims jobs via `POST /api/worker/jobs/start` and runs the **agent image** (`terarchitect-agent`) for each job. The worker materializes an AgentHub-backed workspace for the explicit `BASE_LEAF_ID`/`BASE_HASH`, runs the Director + worker, and publishes the attempt without bind-mounting a host repo into the worker container. Run the coordinator in Compose with `docker compose build agent coordinator && docker compose up -d coordinator`, or from the **repo root** as a host process: `PYTHONPATH=/path/to/terarchitect TERARCHITECT_API_URL=... PROJECT_ID=... python -m coordinator`. See [docs/RUNBOOK.md](../docs/RUNBOOK.md) and [docs/PHASE1_WORKER_API.md](../docs/PHASE1_WORKER_API.md).
